...
void join_all_threads(int conn_no)
{
    ...
    if (to_join[conn_no] != NULL) {
        ...
        if (pthread_join(thread_ids[i], NULL)!=0) {
            if (errno==EDEADLK) fail_errno("Deadlock detected: 2 threads tried to join with each other");
            if (errno==EINVAL)  fail_errno("Another thread is already waiting to join with this thread");
        }
        no_free_threads++;
...
void join_prev_thread(int thrd_no)
{
    ...
    if (to_join[thrd_no] != NULL) {
	  ...
	  pthread_mutex_lock(&threads_mutex);
        ...
        no_response_threads[conn_no]--;
        connection_no[i]=FREE_SLOT;
        debug("\t ... join_prev_thread(%d): joining with %zu, connection %d\n", thrd_no, i, conn_no);
        debug("end of join_prev_thread(%d): was not first\n", thrd_no);
        pthread_mutex_unlock(&threads_mutex);
    }
    else debug("end of join_prev_thread(%d): was first\n", thrd_no);
...
char *get_mime_type(char *filename) {
    ...
    if (pthread_mutex_lock(&mime_mutex) != 0)
        fail_errno("Pthread_mutex_lock Error");
    ...
    if (pthread_mutex_unlock(&mime_mutex))
        fail_errno("Pthread_mutex_unlock Error");
